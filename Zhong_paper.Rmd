...
```{r}
mod3 <- lm(data = data_tf, yes_pct ~ medage + log_medincome + white_pct + log_public_pct + time_pct + trump_pct + sqrt_voter_turnout + plan_yes + current_yes)
summary(mod3)
```
...
```{r}
mod5 <- lm(data = data_tf, yes_pct ~ medage + log_medincome + white_pct + log_public_pct + time_pct + trump_pct + sqrt_voter_turnout + plan_yes * current_yes)
summary(mod5)
anova(mod3,mod5)
```
...
```{r generate model 5 table, echo = F, results = 'asis'}

stargazer::stargazer(list(mod5), 
               title = "Linear regression with interaction results", 
               label = "interactionResults",
               header = F,
               notes = "Linear regression with interaction result",
               notes.align = "l")
```
...
```{r}
mod6 <- lm(data = data_tf, yes_pct ~ medage + log_medincome + white_pct + log_public_pct + time_pct + trump_pct + sqrt_voter_turnout + plan_yes : current_yes)
summary(mod6)
```


```{r generate model 6 table, echo = F, results = 'asis'}

stargazer::stargazer(list(mod6), 
               title = "Linear regression with interaction-only results", 
               label = "interactionOnlyResults",
               header = F,
               notes = "Linear regression with interaction-only result",
               notes.align = "l")
```