...
```{r}
ggplot(mod1, aes(x = predict(mod1),y =resid(mod1))) +
      geom_point() +
...
### Predicted values
```{r}
values_compare <- final_data %>%
   mutate(predicted = predict(mod1, newdata = final_data)) %>%
   select(yes_pct, predicted)
...
```{r}
mod5 <- lm(data = data_tf, log(yes_pct/(1-yes_pct)) ~ medage + log_medincome + white_pct + log_public_pct + time_pct + trump_pct + sqrt_voter_turnout + current_plan)
summary(mod5)
```
