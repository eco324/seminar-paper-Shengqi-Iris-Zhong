...
The datasets above are measured at the precinct level. To match with census data, they will be transformed into census tracts by using shapefiles. Additionally, the shapefile contains a typo in the name of the precinct, and is fixed in the code trunk below. 
...
precinct_tidy_transformed <- vote_allocation_shares %>%
  ...
  group_by(GEOID) %>%
  summarize(pct_trump = sum(trump_share) / sum(election_voter_share),
            voter_turnout = sum(voter_share) / sum(total_voter_share),
            pct_yes = sum(supporter_share) / sum(voter_share))
```
